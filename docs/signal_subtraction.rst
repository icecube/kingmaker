...
The :class:`~kingmaker.pdf.MarginalizedKingPDF` class evaluates this integral by
pre-building a four-dimensional grid over
:math:`(\delta_\mathrm{source},\, \log_{10}\alpha,\, \beta,\,
...
Construct a :class:`~kingmaker.pdf.MarginalizedKingPDF` with the source
declinations and any desired grid options. The only required argument is
``source_declination``. See :ref:`signal-subtraction-grid-options` for the
...
The grid is built once during :meth:`~kingmaker.pdf.MarginalizedKingPDF.__init__`
...
.. _signal-subtraction-interfaces:

Choosing between ``pdf()`` and ``evaluate()``
---------------------------------------------

:class:`~kingmaker.pdf.MarginalizedKingPDF` exposes two methods for evaluating
...
:meth:`~kingmaker.pdf.MarginalizedKingPDF.evaluate` ``(source_decs, event_decs, alpha, beta)``
    ...
    result is compact even for large event and source counts. This is the
    method to use in analyses. On repeat calls with the same event and source
    positions but different ``alpha``/``beta`` values, pass the result of a
    previous call as ``mask=`` to skip the angular-masking loop and go
    straight to the interpolation step.
...
β ∈ [1.5, 8]), the defaults are sufficient.
